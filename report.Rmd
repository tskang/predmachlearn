...
We build a predictive model for activity recognition using the random forest algorithm.
...
The trained random forest will now use the validation data set to etimate the out of sample error rate. The data set was originally part of the total training set, but it was carved out untouched during variable selection, training and tuning of the random forest. Therefore, the validation set should be an unbiased estimate of the random forest's prediction accuracy.
In the following code, we estimate the performance of the model on the validation data set.  
```{r, cache = T}
...
Based on the missclassificaiton rate on the validation set, an unbiased estimate of the random forest's out-of-sample error rate is `r 100*oose`%.
